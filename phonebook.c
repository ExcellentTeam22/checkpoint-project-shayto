...
void insert(void)
{
	...
	while(ans == 'y'){
		if(curr_num_of_contacts == TOTAL_CONTACTS)
 			curr_num_of_contacts = 0;
		else{
			printf("\n\nData of Contact %2.2d{\n",curr_num_of_contacts + 1);
			printf("\n\t  1-F.Name: ");
			len_size = get_input(A[curr_num_of_contacts].fname,size);

			printf("\t  2-L.Name: ");
			len_size = get_input(A[curr_num_of_contacts].lname,size);

			printf("\t  3-Tele.P: ");
			len_size = get_input(A[curr_num_of_contacts].telep,size);

			printf("\t  4-Cell.P: ");
			len_size = get_input(A[curr_num_of_contacts].cellp,size);
    ...
    free(string);
	printf("\n\nYou have inserted ( %d ) contact!\nPress a key to return main page & continue program|-->", curr_num_of_contacts);
	getc(stdin);
}
 //------------------------------------------------------
void delet(void)
{
    size_t size = 31;
	char dfname_string[size], dlname_string[size];
	char *input = NULL;
	size_t len=0;
	size_t len_size=0;
	...
	printf("\n\n  ::Enter first name: ");
	len_size = get_input(dfname_string, size);

	printf("\n  ::Enter last name: ");
	len_size = get_input(dlname_string, size);

	for (i = 0; i < curr_num_of_contacts; i++) {
	 if (strcmp (dfname_string, A[i].fname) == 0 && strcmp (dlname_string, A[i].lname) == 0 ) {
		...
		getc(stdin);
		if(ch == 'y'){
	   	for(j = i ; j < curr_num_of_contacts ; j++)
			A[j] = A[j+1];
	...
	for (i=0; i < curr_num_of_contacts; i++) {
	 if (strcmp (dfname, A[i].fname) == 0 && strcmp (dlname, A[i].lname) == 0 ) {
		...
		if(ch=='y'){
			...
			len_size = get_input(A[i].fname, size);

			printf(" >|Enter new last name: ");
			len_size = get_input(A[i].lname, size);

			printf(" >|Enter new telephone number: ");
			len_size = get_input(A[i].telep, size);

			printf(" >|Enter new cellphone number: ");
            len_size = get_input(A[i].cellp, size);
...
void sortf(void)
{
	struct contact B;
	register int i,j;
	for(i= curr_num_of_contacts-1 ;i > 0; i--)
		for(j = 0; j < i; j++)
...
size_t get_input(char *dst, size_t max_size)
{
    char *input = NULL;
    size_t len = 0;
    size_t len_size = 0;//size getline gets
